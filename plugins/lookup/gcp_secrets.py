...
# References:
#   - API: https://cloud.google.com/secret-manager/docs/reference/rest/v1/projects.secrets/list
...
lookup: gcp_secrets
description:
  - This lookup returns a list of secrets stored in GCP Secret Manager provided
    the caller has the appropriate permissions to read the secret metadata. This 
    does not return secret payloads, only list of secret's resource names.
requirements:
  - python >= 3.0
  - requests >= 2.18.4
  - google-auth >= 1.3.0
...
class GcpSecretsLookup:
    def run(self, method, **kwargs):
        ...
        auth = GcpSession(fake_module, "secretmanager")
        secrets = self.list_secrets(fake_module, auth)
        result = [secret['name'] for secret in secrets]
        return [result]

    def list_secrets(self, module, auth):
        url = "https://secretmanager.googleapis.com/v1/projects/{project}/secrets".format(
            **module.params
        )
...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, **kwargs):
        if not HAS_GOOGLE_CLOUD_COLLECTION:
            raise AnsibleError(
                "gcp_secrets lookup needs a supported version of the google.cloud collection installed. Use `ansible-galaxy collection install google.cloud` to install it"
            )
        return GcpSecretsLookup().run(terms, variables=variables, **kwargs)
